import { Divider, FormControl, FormControlLabel, Grid2, Radio, RadioGroup, Typography, CircularProgress, Alert } from '@mui/material';
...
const RestaurantDetails = () => {
  ...
  useEffect(() => {
    dispatch(getMenuItemsByRestaurantId({
      token,
      id,
      vegeterian: foodType === "veg",
      nonVegeterian: foodType === "non_veg",
      seasonal: foodType === "seasonal",
      category: selectedCategory
    }));
  }, [selectedCategory, foodType, dispatch, id, token]);

  if (restaurant.loading || menu.loading) {
    return <div className='flex justify-center mt-10'><CircularProgress /></div>;
  }

  if (restaurant.error || menu.error) {
    return <div className='flex justify-center mt-10'><Alert severity='error'>Failed to load data. Please try again later.</Alert></div>;
  }
      ...
      <div className='px-5 lg:px-20'>
        ...
        <section className='pt-[2rem] lg:flex relative'>
          ...
          <div className='space-y-5 lg:w-[80%] lg:pl-10'>
            {menu.menuItems.map((items) => <MenuCard key={items.id} items={items} />)}
          </div>
        </section>
      </div>
  );
